...
`r knitr::kable(aggregate(PlantGrowth$weight, by = list(PlantGrowth$group),
mean), "pandoc", col.names = c("Factor level", "Average weight"))`
...
boxplot(weight ~ group,                             # compare weight by group
		...
		col = c("yellow3","green3","blue3"))        # set bar fill colors
```
...
Since the p-value of 0.016 is less than the significance threshold of 0.05, the null hypothesis can be rejected and the difference in treatments is statistically significant. 